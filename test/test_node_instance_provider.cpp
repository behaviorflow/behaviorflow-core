...
class NodeInstanceProviderTest : public ::testing::Test {
 ...
 protected:
	void SetUp() override {
		...
		flag_ = false;
		registrar.registerSimpleNodeType("DummyNode", [](){});
		registrar.registerSimpleNodeType("SetFlagTrue", [this](){flag_ = true;});
		provider = std::make_unique<NodeInstanceProvider>(std::move(registrar));
	}
